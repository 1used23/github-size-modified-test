...
const checkForRepoPage = async () => {
  ...
  let items = document.querySelectorAll(
    "table tbody tr.js-navigation-item td:nth-child(2) a"
  );
  let ageForReference = document.querySelectorAll(
    "table tbody tr.js-navigation-item td:last-child"
  );
  ...
  if (!list) {
    await new Promise((resolve, reject) => {
      setTimeout(function() {
        ...
        ageForReference = document.querySelectorAll(
          "table tbody tr.js-navigation-item td:last-child"
        );
      }, 1000);
    });
  ...
  const inner = `<select id="selectMeasure" class="selectMeasure"><option value="auto" selected>AUTO</option>${MEASURE.map(
    el => {
      return `<option value="${el}">${el}</option>`;
    }
  )}</select>`;

  document.querySelector(".BtnGroup").insertAdjacentHTML("beforebegin", inner);
  ...
  for (let item of items) {
    ...
    if (t === "dir") {
      ...
      label = getHumanReadableSize(t, "auto");
    }
  ...
  const selectedMeasureEl = document.getElementById("selectMeasure");

  selectedMeasureEl.addEventListener(
    "change",
    e => {
      const measureType = e.target.value;
      console.log(e.target.value);
      allSizeNodes = document.querySelectorAll(".github-repo-size-td");
      allSizeNodesArr = Array.from(allSizeNodes);

      for (let i = 0; i < allSizeNodesArr.length + 1; i++) {
        if (isNaN(fileSizes[i])) {
          allSizeNodesArr[i].innerHTML = "...";
        } else {
          if (measureType == "auto") {
            const index = Math.floor(Math.log(fileSizes[i]) / Math.log(K));
            allSizeNodesArr[i].innerHTML =
              parseFloat((fileSizes[i] / Math.pow(K, index)).toFixed(2)) +
              " " +
              MEASURE[index];
          } else {
            allSizeNodesArr[i].innerHTML =
              parseFloat(
                fileSizes[i] / Math.pow(K, MEASURE.indexOf(e.target.value))
              ) +
              " " +
              e.target.value;
          }
      ...
      for (let item of items) {
        ...
        const td = document.querySelector(".css-truncate");
        ...
        if (t === "dir") {
          ...
          label = getHumanReadableSize(t, e.target.value);
        }

        td.innerHTML = `<span class="css-truncate css-truncate-target github-repo-size-td">sadasdsad</span>`;
      }
    },
    false
  );
...
const loadFolderSizes = async () => {
  const files = document.querySelectorAll(
    "table tbody tr.js-navigation-item:not(.up-tree) td.content a"
  );
  const folderSizes = document.querySelectorAll(
    "td.github-repo-size-folder > span"
  );
  const liElem = document.getElementById(LI_TAG_ID);
  ...
  if (data.truncated) {
    console.warn(
      "github-repo-size: Data truncated. Folder size info may be incomplete."
    );
  }
